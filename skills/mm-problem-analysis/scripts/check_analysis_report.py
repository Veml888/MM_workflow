...
规范（mm-problem-analysis SKILL §三 Step 6 + references/quality-gates.md）：
  docs/01-analysis-report.md 必须包含九部分（## 0 阶段GATE ~ ## 8 输出契约与下游交接），
  每问有稳定 ID（REQ-###/P-##/DS-###/MC-P##-##），每个首选模型可证伪（有选择证据、局限与淘汰条件），
  且 Q-PA1~Q-PA5 门禁已填写（允许 N/A 仅限 Q-PA4），
  且报告至少引用 G-PA / G-READ / G-SEM 之一。
...
def check(text, path):
    ...
    # 5) 引用 quality-gates.md：脚本与该 references 对齐；如报告出现 G-PA1~4 自查行，
    #    仅做"至少一处引用 G-PA / G-READ / G-SEM 之一"的轻量校验（不强校验状态）
    if not re.search(r"G-PA[1-4]|G-READ|G-SEM", text):
        errs.append("报告未引用 G-PA / G-READ / G-SEM 任何一项（应在阶段 GATE 区体现）")
