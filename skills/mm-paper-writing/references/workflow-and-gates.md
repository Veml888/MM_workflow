...
本 skill 在完整工作流中由 `mm-orchestrator` 调度为**写作轮唯一一次完整调用 + 独立审查轮**。写作轮必须从头完整读取本文件和本次适用的直接引用规范，读取已定稿的结构规划、页面预算、初稿审计、图件清单、全部报告和结果，并在本次调用内完成多轮终稿、双遍编译、正文页数与全部专项审计；独立审查轮对成稿做独立终审与修订（见下）。它不是“打开初稿快速润色后结束”的工具。

### 写作与审查协议（进入验收前的硬门禁）

1. **完整读取只在写作轮发生一次**：写作轮开始时运行 `scripts/read_complete.py plan`，按 `writing-order.json` 的必读清单逐块读至全部文件末尾并确认每块出现 `READ-END`，生成读取回执（`paper/skill-read-receipt.json`）并通过 `verify`。读取完整性由回执硬门禁保证，独立审查轮不重新完整读取。
2. **写作轮（成稿）**：从上游材料新建或重构 `paper/论文.tex`，落实全部注册模块的适用要求，完成至少双遍编译和全部专项门禁。生成读取回执与 `paper/writing-audit.md`，按每个必读文件及其全部二/三级标题记录“已执行/不适用（含理由）/阻断”，并通过 `validate_requirement_coverage.py`；登记回执和审计文件哈希，此时 `paper_final.status` 只能保持 `in_progress`，不得置为 `complete`。
3. **独立审查轮（终审与修订）**：优先由**未参与写作的独立会话或子代理**执行（真实 fresh eyes；无条件时由主会话按冷读清单执行并在审计注明），以写作轮终稿为对象：① 就地重读本轮实际修订章节对应的 `reread_before_writing` 模块，涉排版改动加读 `layout-and-compilation.md`；② 对照 `docs/01~03`、`results/`、`paper/structure-plan.md` 追踪表逐问核对（数字一致、题面逐条覆盖、最终回答可定位），未改动项也逐项记录证据；③ 跑全套专项机检，发现问题直接修正、重编译双遍并重跑。**任何修订动笔前，必须先核对修订对象所属 owner 章节模块的全文；凡修订对象属于模块中显式豁免/例外条款（如符号说明表、附录清单表、源码行），处置须引用该例外原文作依据，禁止以通用规则（如"每表必引用"）覆盖显式例外——因违反例外条款产生的"修复"视为新缺陷，按高严重度回写。** 产出 `paper/review-findings.md`（逐章发现：问题/严重度/证据/处置）与 `paper/review-audit.md`（覆盖注册表每个必读文件并通过 `validate_requirement_coverage.py`）。
...
| G-2 图表齐全 | 按 manifest 检查适用的 `figures/`、`docs/04`、`docs/05-diagrams-report.md` | 列出实际文件清单；按 `paper/figure-requirements.md` 核对适用的非数据图；缺图或写作中临时新增图，先回写对应 skill |
...
### Step 2.5：模型建立前的图件需求预登记（示意图/流程图必须动笔前识别）

模型建立节的示意图、机理图、符号定义图、推导流程图，其意义在于"帮读者理解公式/符号/推导"，因此**必须在动笔写引用它的正文之前就存在**。默认**保守规划**：这些图**主要落在"模型的建立与求解"章节**（为该章的公式/符号/约束/算法配图），只登记**确定且大概率会被正文引用**的图，可画可不画的不强画（标 planned 待定，写作中确认需要再走路径 B 补）。"该画什么图/要不要画"的决策原则以 `mm-graphics` §二"决定要画哪些非数据图"为准。允许两条路径：
- **路径 A（默认·先画后写）**：动笔写模型建立前，逐问识别需要哪些图，登记进 `paper/figure-requirements.md`，交由 `mm-graphics` 画好，再回头写正文；正文只用 `\ref` 引用。
- **路径 B（写作中返回补画·允许）**：写作进行到某处，发现某公式/符号需要配图、且尚未登记/未画，**允许停止该处撰写，返回对应图件 skill 补画**（登记→画→产出），画好后再回到正文继续写该处并用 `\ref` 引用。
- **硬性判据（两路径共用）**：在动笔写**引用该图的这段文字**之前，图必须已完成（`figures/` 有产出、`figure-requirements.md` 该图状态为 `drawn`）。严禁整段/整章写完再补图当"事后装饰"；严禁用正文文字替代本该配图的位置；写作中临时发现需要图，也必须走路径 B 先补、再写文字。

按 `chapters/05-模型的建立与求解.md` 的"公式/符号的图形化辅助"节逐问识别，登记三项必填：
- **拟配图清单**：本问哪些公式/符号/约束有直观几何/结构/分布/状态含义值得配线稿图（如"几何关系的夹角定义图""统计判定规则的接受域/拒绝域图""递归/循环结构的流程图"）；哪些**明确不配**（纯代数、看公式即懂，如几个量相乘、递归求和闭式）。
- **每图的核心论证目标**：这张图帮读者看懂哪一个符号/公式/推导步骤的具体意义（一句话）。
- **候选正文锚点**：拟放在哪个公式定义处（对应 figure-requirements 的"非数据图"区）。
- **状态**（`planned` / `drawn` / `in-text`）：识别出需要→`planned`；图件 skill 产出→`drawn`；正文 `\ref` 已落→`in-text`。状态写进 `figure-requirements.md`，作为"图是否先于文字存在"的可反查凭证。

登记后：若对应图尚未产出，走路径 B——**回写给负责的图件 skill（返回调用）**，按 `figure-requirements.md` 的"负责 skill"列判断：数据图（折线/柱状/散点/热力/曲面/灵敏度等，展示真实结果）回写 `mm-figures`；非数据图（逻辑/机理/结构/场景/符号定义/推导流程图）回写 `mm-graphics`。待 `figures/` 有对应 PDF 且更新 `docs/04-figures-report.md`（数据图）或 `docs/05-diagrams-report.md`（非数据图）、`figure-requirements.md` 状态置 `drawn` 后再动笔写该处正文。写作时只用 `\ref` 引用这些图（见 3b 条），图注编号与全文连续。
...
- 插图按 `docs/04`、`docs/05-diagrams-report.md` 说明插入，图注/表注全文连续编号；结果表线型按内容选择（三线表或全框线表，符号说明表固定三线表，见 `layout-and-compilation.md`"表格线型"）。写结果与插图前必须再次读取 `layout-and-compilation.md`。**写正文前先核对 `figures/` 与 `docs/04`/`docs/05` 是否齐全**，缺图或写作中临时发现新图需求时，先停止该处撰写、回写对应画图 skill，待图产出并更新报告后再继续；不在本阶段自行生成。
...
- **问题分析与数据预处理分离**：问题分析章可以概括数据来源、可用性、质量风险、必要处理方向及其理由，但不写具体缺失率、异常值数量、处理参数、特征构造结果或描述统计结果。多问共享同一数据时，在问题一章之前设置条件性的公共数据处理一级小节；各问数据独立时分别写入对应问题；部分共享时采用“公共处理 + 问题专属处理”。
...
- **问题重述与分析的自然篇幅**：问题背景和逐问重述按题目数量、对象复杂度及输入输出边界自然展开，不设置最低占页率；`1.1 问题背景`不得只写可替换到任意题目的短套话。问题分析只有在缺少任务边界、数学类型、主要困难、数据条件、方法依据、跨问依赖、验证方式或交付形式时才补写，不得为填页重复题意。具体执行 `chapters/01-问题重述.md` 与 `chapters/02-问题分析.md`。**若正文整体不足 25 页，按 `篇幅控制-playbook.md` 处理，不要把"补篇幅"误解为"只能加长问题重述/问题分析"——那两章恰恰是最不该注水的**。

**输出要求**：摘要页、图表数量、正文25～30页、字体、数字一致和匿名要求均按注册模块执行；跨阶段真实性与追踪遵循共享不变量。写作轮即使全部脚本通过也只能保持 `paper_final.status=in_progress`。只有独立审查轮完成（`review-findings.md` 无阻断项、逐问核对与全套机检通过、`review-audit.md` 覆盖注册表每个必读文件并通过 `validate_requirement_coverage.py`）、`check_paper_length.py` 与全部专项脚本 exit 0、`read_receipt`、`writing_audit`、`review_findings`、`review_audit`、`page-audit.json`、`content-gap-report.md` 均已生成并登记哈希、写作轮至少双遍稳定编译（审查轮有修订时修订后再双遍）时，才能把 `paper_final.status` 置为 `complete`。同时登记初稿模式/SHA256/基线页数、计划页数、最终正文页数、页数审计路径、缺口报告路径和编译遍数。**审查轮不是只读抽查或快速润色**：以写作轮产出的 `paper/论文.tex|pdf` 为当前终稿（不得改名 `draft-baseline`），对未改动的要求也必须逐项重新核实并记录证据，而非沿用写作轮结论；发现问题直接修正并重跑全部机检。**修订合并纪律（硬性）**：写作轮内部多轮修订与本轮修订一律以整节合并重写实现——先重读整节、合并同类段（同一信息全文至多出现一次），禁止以追加段落方式修订；修订节重跑以节为单位的冷读（删除无新增信息段、合并同义句）。
