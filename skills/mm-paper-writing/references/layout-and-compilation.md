...
- **内部多轮要求**：完成全部正文并最后写摘要后，至少双遍编译；随后依次运行页面长度、字体/表格/结构、引用、布局、摘要和语言审计。任一脚本失败都不得结束本次调用。正文页数不足或章节明显偏离预算时，生成/更新 `paper/content-gap-report.md`，按缺失的推导、参数来源、检验、最终回答等逐项补充实质内容，再次双遍编译和复检。
