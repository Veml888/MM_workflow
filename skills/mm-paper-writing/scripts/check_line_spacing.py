...
规范：正文（摘要页之后、附录之前）行距必须为 linespread 1.25 的排版效果，
即 12pt 字号下相邻两行基线距约 17.5pt（对应 Word 约 1.25 倍行距）；
实测中位数须落在 [16.5, 19] pt 内。
附录保持单倍行距，不参与本检查。

用法：python check_line_spacing.py <论文.pdf> [--min-pt 16.5] [--max-pt 19] [--output out.json]
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--min-pt", type=float, default=16.5)
    parser.add_argument("--max-pt", type=float, default=19.0)
