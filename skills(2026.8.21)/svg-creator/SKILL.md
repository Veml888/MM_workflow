...
name: svg-creator
description: >
  Create hand-drawn style SVG flowcharts and diagrams for drafts, whiteboard notes,
  presentations and informal visual explanations. Supports aspect ratio specification
  (16:9, 4:3, 1:1, A4, or custom W:H ratios). Trigger when the user asks for a
  hand-drawn/sketch-style flowchart, diagram, architecture diagram, process chart, mind map,
  or any visual representation of a workflow/system using SVG. Do NOT use for math modeling
  competition papers - use mm-diagrams or mm-visual-concept (black-white-gray academic style) instead.
---

> **用途边界（2026-08 更新）**：本 skill 只负责**手绘/马克笔风格**示意图，适合草稿、讲解、非正式展示。**数学建模竞赛论文一律禁用本 skill**——论文中的流程图、机理图、纯视觉示意图必须用 `mm-diagrams` 与 `mm-visual-concept`（黑白灰学术风，手写 SVG 精确布点）。若用户要求"论文图 / 数模图 / 放进论文的图"，不要使用本 skill。
