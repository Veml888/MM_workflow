...
│   ├── 05-diagrams-report.md      # 流程图/机理图说明（mm-diagrams）
│   ├── 05-visual-report.md        # 纯视觉示意图说明（mm-visual-concept）
...
│   └── *.png（可选 *.svg）         # 纯视觉示意图
└── paper/                         # 论文（LaTeX 为主）
    ├── structure-draft.md         # 第一遍：章节结构草稿
    ├── figure-requirements.md     # 第一遍：插图需求与正文锚点
...
| 论文结构草稿 | `docs/01~03`、`results/` | `paper/structure-draft.md`、`paper/figure-requirements.md`、`paper/writing-gates.md`（初始 pending） |
| 图表生成 | `docs/03-results-report.md`、`results/`、`paper/figure-requirements.md` | `figures/*.pdf|svg|png`、`docs/04-figures-report.md` |
| 流程架构图 | `docs/01~03`、`paper/figure-requirements.md` | `figures/*.tex|pdf|png`（可选 `*.svg`）、`docs/05-diagrams-report.md` |
| 纯视觉示意图 | 题面、`docs/01~03`、`paper/figure-requirements.md` | `figures/*.png`、可选 `*.svg`、`docs/05-visual-report.md` |
| 论文终稿 | 结构草稿 + 全部报告 + `figures/` + `results/` + G-1~G-5 全部 PASS | `paper/论文.tex`、`paper/论文.pdf` |
